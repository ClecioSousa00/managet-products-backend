...
export class UpdateProductUseCase implements UseCase<InputDto, OutputDto> {
  ...
  async execute(input: InputDto): Promise<OutputDto> {
    ...
    if (input.name) product.updateName(input.name)

    if (input.quantity && input.quantity >= 0)
      product.changeQuantity(input.quantity)

    if (input.salePrice) product.changeSalePrice(input.salePrice)

    if (input.purchasePrice) product.changePurchasePrice(input.purchasePrice)
